...
export default function CreatorOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
        ...
        console.log('📋 Creator orders response:', response);
        
        if (response.success && response.orders) {
          // Debug: Log the first order to see brand structure
          if (response.orders.length > 0) {
            console.log('📋 Creator Orders - First order brand data:', {
              brand: response.orders[0].brand,
              company_name: response.orders[0].brand?.company_name,
              user_name: response.orders[0].brand?.user?.name,
              brand_user: response.orders[0].brand?.user,
              full_brand_object: JSON.stringify(response.orders[0].brand, null, 2)
            });
          }
          ...
          const transformedOrders: Order[] = response.orders.map((order: {
            ...
            const brandName = order.brand?.company_name === 'Company Name' 
              ? (order.brand?.user?.name || `Brand from ${order.brand?.location_city || 'Unknown Location'}`)
              : (order.brand?.company_name || order.brand?.user?.name || order.brand?.fullName || 'Unknown Brand');
            
            // Debug: Log brand name calculation
            console.log('📋 Creator Orders - Brand name calculation:', {
              company_name: order.brand?.company_name,
              user_name: order.brand?.user?.name,
              location_city: order.brand?.location_city,
              calculated_brand_name: brandName
            });
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (response.success && response.orders) {
        // Debug: Log the first order to see brand structure during refresh
        if (response.orders.length > 0) {
          console.log('📋 Creator Orders Refresh - First order brand data:', {
            brand: response.orders[0].brand,
            company_name: response.orders[0].brand?.company_name,
            user_name: response.orders[0].brand?.user?.name,
            brand_user: response.orders[0].brand?.user,
        ...
        const transformedOrders: Order[] = response.orders.map((order: {
          ...
          // Debug: Log brand name calculation during refresh
          console.log('📋 Creator Orders Refresh - Brand name calculation:', {
            company_name: order.brand?.company_name,
            user_name: order.brand?.user?.name,
            location_city: order.brand?.location_city,
            calculated_brand_name: brandName
  ...
  const handleViewOrder = (order: Order) => {
    ...
    setSelectedOrderId(order.id.replace('#', ''));
  ...
  const handleOrderAction = () => {
    // Refresh orders when an action is performed
    handleRefresh();
  };
                      ...
                      <td colSpan={5} className="px-4 sm:px-6 py-8 sm:py-12 text-center">
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-sm sm:text-base text-gray-500 mb-3 sm:mb-4 px-4">
                            {activeTab === 'New' ? 'No new orders waiting for your approval' :
                          ...
                          <p className="text-sm sm:text-base text-gray-500 px-4">
                            Orders will appear here when brands purchase your packages.
                          </p>
          </div>
                      </td>
                    </tr>
